(function () {
  ...
  var _3 = function (schema, options) {
    ...
    function iterate_object_props(base, parent) {
      ...
      let isArray = Array.isArray(base); //start the struct

      let struct = isArray ? "[" : "{"; //iterate the properties
      ...
      for (let k in base) {
        ...
        let isNumeric = isFinite(k); //the actual path to be combined with previous parents if it's the case

        let actual = isNumeric ? "[" + k + "]" : "." + k; //key var is for the struct and only needed when it's an object
        //for example, { message: 'ok' }, the prop "message" is the key
        //another example, ['ok', 'ok', 'deny'], here there is no key to add

        let key = isArray ? "" : '"' + k + '":'; //the magic occurs here, checking values and types, in this way
        ...
        if (base[k] === null) {
          ...
          struct += key + "null,"; //struct var here will be '[null,'
