...
(function() {
  ...
  like.json = function(schema, options) {
    ...
    function value(v, k) {
      ...
      if (typeof v === "number" || v instanceof Number) {
        return (
          "' + " +
          (options.finite ? "(isFinite(" + k + ") ? " + k + " : null)" : k) +
          " + '"
        );
      }
    ...
    function iterate_object_props(base, parent) {
      ...
      for (let k in base) {
        ...
        if (base[k] === null) {
          ...
          ["function", "symbol", "undefined"].indexOf(typeof base[k]) !== -1
        ) {
          ...
          struct +=
            key + "\"' + " + "o" + parent + actual + ".toJSON() + '\"" + ",";
        } else if (
          typeof base[k] === "object" &&
          [String, Number, Boolean].indexOf(base[k].constructor) === -1
        ) {
    ...
    if (
      ...
      [String, Number, Boolean].indexOf(schema.constructor) === -1
    ) {
      return Function(
        "o",
        "return '" + iterate_object_props(schema, "") + "';"
      );
    }
  ...
  let $unique_reference = 0;
  like.stringify = function(obj, id, options) {
    if (!id) {
      for (const key in this._json_cache) {
        if (this._json_cache[key].$reference === obj) {
          return this._json_cache[key].$cache(obj);
        }
      }

      this._json_cache[--$unique_reference] = {
        $reference: obj,
        $cache: this.json(obj, options)
      };

      return this._json_cache[$unique_reference].$cache(obj);
    }
  ...
  //   if (!this._json_cache[obj]) {
  //     this._json_cache[obj] = this.json(obj, options);
  //   }
  //   return this._json_cache[obj](obj);
  // };
